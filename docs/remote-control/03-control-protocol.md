...
**Status (2026-03-14):** Tier 0 local protocol, Tier 1 single-session relay routing, the Tier 2 spawn/load identity skeleton, Tier 3 delivery/permission semantics, Tier 4 human observe/inject, and initial Tier 5 takeover control semantics are implemented for `auth`, `initialize`, `registry.list_sessions`, `session.status`, `session.spawn`, `runtime.stop`, `session.load`, `session.prompt`, `session.cancel`, `session.permission_response`, `session.attach`, `session.detach`, `session.agent_ready`, and normalized `session.update`. The confirm-style permission lifecycle is protocol-shaped end-to-end, pending permissions are visible in `session.status` and `registry.list_sessions`, an attached human in `inject` or `takeover` mode holds permission authority, attachment-related `status` updates can carry explicit action details like `attached`, `detached`, and `mode_changed`, and orchestrator prompts can now be acknowledged as queued while takeover is active. Full PTY byte transport (`pty.output`, `pty.input`, `pty.resize`) and richer host routing remain future tiers.
