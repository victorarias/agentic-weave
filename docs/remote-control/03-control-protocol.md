...
**Status (2026-03-14):** Tier 0 local protocol, Tier 1 single-session relay routing, the Tier 2 spawn/load identity skeleton, Tier 3 delivery/permission semantics, and an initial Tier 4 human observe/inject slice are implemented for `auth`, `initialize`, `registry.list_sessions`, `session.status`, `session.spawn`, `runtime.stop`, `session.load`, `session.prompt`, `session.cancel`, `session.permission_response`, `session.attach`, `session.detach`, `session.agent_ready`, and normalized `session.update`. The confirm-style permission lifecycle is protocol-shaped end-to-end, pending permissions are visible in `session.status` and `registry.list_sessions`, an attached human in `inject` mode now holds permission authority, and attachment-related `status` updates can carry explicit action details like `attached`, `detached`, and `mode_changed`. Full takeover/PTY transport and richer host routing remain future tiers.
