...
**Status (2026-03-14):** Tier 0 local protocol, Tier 1 single-session relay routing, the Tier 2 spawn/load identity skeleton, Tier 3 delivery/permission semantics, Tier 4 human observe/inject, and initial Tier 5 takeover semantics plus PTY byte transport are implemented for `auth`, `initialize`, `registry.list_sessions`, `session.status`, `session.spawn`, `runtime.stop`, `session.load`, `session.prompt`, `session.cancel`, `session.permission_response`, `session.attach`, `session.detach`, `pty.input`, `pty.resize`, `session.agent_ready`, `pty.output`, and normalized `session.update`. The confirm-style permission lifecycle is protocol-shaped end-to-end, pending permissions are visible in `session.status` and `registry.list_sessions`, an attached human in `inject` or `takeover` mode holds permission authority, attachment-related `status` updates can carry explicit action details like `attached`, `detached`, and `mode_changed`, orchestrator prompts can be acknowledged as queued while takeover is active, and a scripted PTY-backed runtime can now stream `pty.output` and accept `pty.input` / `pty.resize`. Switching that PTY path from the scripted helper to a real interactive pi runtime and richer host routing remain future tiers.
