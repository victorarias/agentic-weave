...
**Status (2026-03-14):** in progress. The takeover control-plane groundwork is in place (`session.attach` accepts `takeover`, takeover holds permission authority, relay queues orchestrator prompts while takeover is active, and queued prompts flush on detach/de-escalation), and the first PTY byte-transport slice is now working against both a scripted PTY-backed runtime and a real interactive `pi` process launched via wrapper PTY mode: relay forwards `pty.output` only to the attached takeover human and accepts `pty.input` / `pty.resize` from that human. Relay-managed `session.spawn` / `session.load` can explicitly choose that real interactive pi PTY runtime via transport selection, `runtime.replace` can switch a resumable session between runtime transports under a stable `session_id`, and attach intent is now more automatic: `session.attach takeover` upgrades resumable relay-managed sessions to PTY before granting takeover while `weave-inspect relay takeover` follows up with an automatic local-terminal resize so the human sees the pi screen immediately. The remaining gap is polishing de-escalation / return-to-RPC behavior beyond the current explicit `runtime.replace` or `switch-transport rpc` path.
