...
- Treat every real PTY/TUI bug as a candidate permanent fixture in the terminal lab. If a user or agent captures a weird byte sequence, resize issue, redraw bug, or takeover edge case, prefer encoding it under `testdata/terminal/` and `terminaltests/` instead of leaving it as one-off debugging knowledge.
